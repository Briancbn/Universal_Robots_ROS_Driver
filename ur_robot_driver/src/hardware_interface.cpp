...
bool HardwareInterface::initUR(rclcpp::Node::SharedPtr& /*root_nh*/, rclcpp::Node::SharedPtr& robot_hw_nh)
{
  joint_velocities_ = { { 0, 0, 0, 0, 0, 0 } };
  joint_efforts_ = { { 0, 0, 0, 0, 0, 0 } };
  ...
  // Pass the node pointer
  robot_hw_nh_ = robot_hw_nh;

  // The robot's IP address.
  robot_hw_nh_->declare_parameter("robot_ip");
  if (!robot_hw_nh_->get_parameter<std::string>("robot_ip", robot_ip_))
  {
    RCLCPP_ERROR_STREAM(robot_hw_nh_->get_logger(),
                        "Required parameter " << robot_hw_nh_->get_fully_qualified_name() <<
                        "/robot_ip" << " not given.");
    return false;
  }

  // Port that will be opened to communicate between the driver and the robot controller.
  robot_hw_nh_->declare_parameter<int>("reverse_port", 50001);
  int reverse_port = robot_hw_nh_->get_parameter("reverse_port").as_int();

  // The driver will offer an interface to receive the program's URScript on this port.
  robot_hw_nh_->declare_parameter<int>("script_sender_port", 50002);
  int script_sender_port = robot_hw_nh_->get_parameter("script_sender_port").as_int();
  ...
  robot_hw_nh_->declare_parameter<std::string>("script_sender_port", "");
  tf_prefix_ = robot_hw_nh_->get_parameter("tf_prefix").as_string();

  // Optional parameter to change the id of the wrench frame
  robot_hw_nh_->declare_parameter<std::string>("wrench_frame_id", "wrench");
  wrench_frame_id = robot_hw_nh_->get_parameter("wrench_frame_id").as_string();

  // Path to the urscript code that will be sent to the robot.
  robot_hw_nh_->declare_parameter("script_file");
  if (!robot_hw_nh_->get_parameter<std::string>("script_file", script_filename))
  {
    RCLCPP_ERROR_STREAM(robot_hw_nh_->get_logger(),
                        "Required parameter " << robot_hw_nh_->get_fully_qualified_name() <<
                        "/script_file" <<  " not given.");
    return false;
  }

  // Path to the file containing the recipe used for requesting RTDE outputs.
  robot_hw_nh_->declare_parameter("output_recipe_file");
  if (!robot_hw_nh_->get_parameter<std::string>("output_recipe_file", output_recipe_filename))
  {
    RCLCPP_ERROR_STREAM(robot_hw_nh_->get_logger(),
                        "Required parameter " << robot_hw_nh_->get_fully_qualified_name() <<
                        "/output_recipe_file" << " not given.");
    return false;
  }

  // Path to the file containing the recipe used for requesting RTDE inputs.
  robot_hw_nh_->declare_parameter("input_recipe_file");
  if (!robot_hw_nh_->get_parameter<std::string>("input_recipe_file", input_recipe_filename))
  {
    RCLCPP_ERROR_STREAM(robot_hw_nh_->get_logger(),
                        "Required parameter " << robot_hw_nh_->get_fully_qualified_name() <<
                        "/input_recipe_file" << " not given.");
    return false;
  }
  ...
  robot_hw_nh_->declare_parameter("headless_mode");
  if (!robot_hw_nh_->get_parameter<bool>("headless_mode", headless_mode))
  {
    RCLCPP_ERROR_STREAM(robot_hw_nh_->get_logger(),
                        "Required parameter " << robot_hw_nh_->get_fully_qualified_name() <<
                        "/headless_mode" << " not given.");
    return false;
  }
  ...
  robot_hw_nh_->declare_parameter<bool>("non_blocking_read", false);
  non_blocking_read_ = robot_hw_nh_->get_parameter("non_blocking_read").as_bool();

  // Specify gain for servoing to position in joint space.
  // A higher gain can sharpen the trajectory.
  robot_hw_nh_->declare_parameter<int>("servoj_gain", 2000);
  int servoj_gain = robot_hw_nh_->get_parameter("servoj_gain").as_int();
  if ((servoj_gain > 2000) || (servoj_gain < 100))
  {
    RCLCPP_ERROR_STREAM(robot_hw_nh_->get_logger(),
                        "servoj_gain is " << servoj_gain <<
                        ", must be in range [100, 2000]");
    return false;
  }

  // Specify lookahead time for servoing to position in joint space.
  // A longer lookahead time can smooth the trajectory.
  robot_hw_nh_->declare_parameter<double>("servoj_lookahead_time", 0.03);
  double servoj_lookahead_time = robot_hw_nh_->get_parameter("servoj_lookahead_time").as_double();
  if ((servoj_lookahead_time > 0.2) || (servoj_lookahead_time < 0.03))
  {
    RCLCPP_ERROR_STREAM(robot_hw_nh_->get_logger(),
                        "servoj_lookahead_time is " << servoj_lookahead_time <<
                        ", must be in range [0.03, 0.2]");
    return false;
  }
  ...
  program_state_pub_ = robot_hw_nh_->create_publisher<std_msgs::msg::Bool>("robot_program_running", 10);
  ...
  // well.
  robot_hw_nh_->declare_parameter<bool>("use_tool_communication", false);
  bool use_tool_communication = robot_hw_nh_->get_parameter("use_tool_communication").as_bool();
  ...
  if (use_tool_communication)
  {
    ...
    robot_hw_nh_->declare_parameter("tool_voltage");
    if (!robot_hw_nh_->get_parameter<ToolVoltageT>("tool_voltage", tool_voltage))
    {
      RCLCPP_ERROR_STREAM(robot_hw_nh_->get_logger(),
                          "Required parameter " << robot_hw_nh_->get_fully_qualified_name() <<
                          "/tool_voltage" << " not given.");
      return false;
    }
    ...
    robot_hw_nh_->declare_parameter("tool_parity");
    if (!robot_hw_nh_->get_parameter<ParityT>("tool_parity", parity))
    {
      RCLCPP_ERROR_STREAM(robot_hw_nh_->get_logger(),
                          "Required parameter " << robot_hw_nh_->get_fully_qualified_name() <<
                          "/tool_parity" << " not given.");
      return false;
    }
    ...
    // Note: This parameter is only evaluated, when the parameter "use_tool_communication"
    // is set to TRUE.  Then, this parameter is required.
    robot_hw_nh_->declare_parameter("tool_baud_rate");
    if (!robot_hw_nh_->get_parameter<int>("tool_baud_rate", baud_rate))
    {
      RCLCPP_ERROR_STREAM(robot_hw_nh_->get_logger(),
                          "Required parameter " << robot_hw_nh_->get_fully_qualified_name() <<
                          "/baud_rate" << " not given.");
    ...
    // Note: This parameter is only evaluated, when the parameter "use_tool_communication"
    // is set to TRUE.  Then, this parameter is required.
    robot_hw_nh_->declare_parameter("tool_stop_bits");
    if (!robot_hw_nh_->get_parameter<int>("tool_stop_bits", stop_bits))
    {
      RCLCPP_ERROR_STREAM(robot_hw_nh_->get_logger(),
                          "Required parameter " << robot_hw_nh_->get_fully_qualified_name() <<
                          "/tool_stop_bits" << " not given.");
      return false;
    }
    ...
    // Note: This parameter is only evaluated, when the parameter "use_tool_communication"
    // is set to TRUE.  Then, this parameter is required.
    robot_hw_nh_->declare_parameter("tool_rx_idle_chars");
    if (!robot_hw_nh_->get_parameter<int>("tool_rx_idle_chars", rx_idle_chars))
    {
      RCLCPP_ERROR_STREAM(robot_hw_nh_->get_logger(),
                          "Required parameter " << robot_hw_nh_->get_fully_qualified_name() <<
                          "/tool_rx_idle_chars" << " not given.");
      return false;
    }
    ...
    // Note: This parameter is only evaluated, when the parameter "use_tool_communication"
    // is set to TRUE.  Then, this parameter is required.
    robot_hw_nh_->declare_parameter("tool_tx_idle_chars");
    if (!robot_hw_nh->get_parameter<int>("tool_tx_idle_chars", tx_idle_chars))
    {
      RCLCPP_ERROR_STREAM(robot_hw_nh_->get_logger(),
                          "Required parameter " << robot_hw_nh_->get_fully_qualified_name() <<
                          "/tool_tx_idle_chars" << " not given.");
      return false;
    }
  ...
  robot_hw_nh_->declare_parameter<std::string>("kinematics.hash", "");
  std::string calibration_checksum = robot_hw_nh_->get_parameter("kinematics.hash").as_string();
  ...
  RCLCPP_INFO_STREAM(robot_hw_nh_->get_logger(), "Initializing urdriver");
  ...
  {
    RCLCPP_FATAL_STREAM(robot_hw_nh_->get_logger(),
                        e.what() << " See parameter '" <<
                        robot_hw_nh_->get_fully_qualified_name() << "/use_tool_communication'.");
    return false;
  }
  catch (urcl::UrException& e)
  {
    RCLCPP_FATAL_STREAM(robot_hw_nh_->get_logger(), e.what());
    return false;
  }
  ...
  // end
  command_sub_ = robot_hw_nh_->create_subscription<std_msgs::msg::String>("script_command", 10,
                                                                          std::bind(&HardwareInterface::commandCallback, this, _1));

  // Names of the joints. Usually, this is given in the controller config file.
  if (!robot_hw_nh_->get_parameter<std::vector<std::string>>("joints", joint_names_))
  {
    RCLCPP_ERROR_STREAM(robot_hw_nh_->get_logger(),
                        "Cannot find required parameter " <<
                        robot_hw_nh_->get_fully_qualified_name() <<
  ...
  {
    RCLCPP_DEBUG_STREAM(robot_hw_nh_->get_logger(),
                        "Registering handles for joint " << joint_names_[i]);
...
//      new realtime_tools::RealtimePublisher<ur_dashboard_msgs::SafetyMode>(robot_hw_nh, "safety_mode", 1, true));
  ...
  set_speed_slider_srv_ = robot_hw_nh_->create_service<ur_msgs::srv::SetSpeedSliderFraction>("set_speed_slider", std::bind(&HardwareInterface::setSpeedSlider, this, _1, _2, _3));

  // Service to set any of the robot's IOs
  set_io_srv_ = robot_hw_nh_->create_service<ur_msgs::srv::SetIO>("set_io", std::bind(&HardwareInterface::setIO, this, _1, _2, _3));
  ...
  if (headless_mode)
  {
    ...
    resend_robot_program_srv_ =
        robot_hw_nh->create_service<std_srvs::srv::Trigger>("resend_robot_program", std::bind(&HardwareInterface::resendRobotProgram, this, _1, _2, _3));
  }

  // Calling this service will make the "External Control" program node on the UR-Program return.
  deactivate_srv_ = robot_hw_nh_->create_service<std_srvs::srv::Trigger>("hand_back_control", std::bind(&HardwareInterface::stopControl, this, _1, _2, _3));
  ...
  tare_sensor_srv_ = robot_hw_nh_->create_service<std_srvs::srv::Trigger>("zero_ftsensor", std::bind(&HardwareInterface::zeroFTSensor, this, _1, _2, _3));

  ur_driver_->startRTDECommunication();
  RCLCPP_INFO_STREAM(robot_hw_nh_->get_logger(), "Loaded ur_robot_driver hardware_interface");

  // Setup the mounted payload through a ROS service
  set_payload_srv_ = robot_hw_nh_->create_service<ur_msgs::srv::SetPayload>(
      "set_payload",
      [&](const std::shared_ptr<rmw_request_id_t> req_header,
          const ur_msgs::srv::SetPayload::Request::SharedPtr req,
          const ur_msgs::srv::SetPayload::Response::SharedPtr resp) {
        (void)req_header;
